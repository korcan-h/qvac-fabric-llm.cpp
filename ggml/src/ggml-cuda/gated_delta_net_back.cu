...
uint32_t compute_lanes_per_col(const uint32_t S_v, const uint32_t warp_size) {
    uint32_t lanes_per_column =
        (S_v >= 128) ? 8u : cuda::std::min<uint32_t>(S_v, warp_size);
...
gdn_back_threads_per_block(const uint32_t S_v, const uint32_t warp_size) {
    ...
    const uint32_t max_warps = cuda::std::min(
        cuda::std::min(32u, S_v * lanes_per_col / warp_size),
        CUDA_GDN_BACK_MAX_BLOCK_SIZE / warp_size);

    return max_warps != 0u
        ? cuda::std::bit_floor(max_warps) * warp_size
        : cuda::std::min(S_v, CUDA_GDN_BACK_MAX_BLOCK_SIZE);
}
